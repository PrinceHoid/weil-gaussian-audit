# rigorous_weil_sweep.py
# Self-contained certified proof that the prime-side Weil functional Q(t) >= 0 on [11, 1737],
# using outward-rounded interval arithmetic built on IEEE-754 primitives.
...
while i < len(fr) and fr[i][0] <= cur:
    ...
    if j == i: break
    i = j
covered = cur >= Fraction(1737)
print(f"[coverage] union of {len(certs)} certified intervals reaches t = {float(cur)}  "
      f"=> [11, 1737] covered: {covered}")
assert covered

with open('/mnt/user-data/outputs/certificates.csv','w') as f:
    f.write("t_lo,t_hi,delta,pair_Q_lowerbound,threshold_L2d2_over_8,certified_margin\n")
    for a,b,d,q,th in certs:
        f.write(f"{a!r},{b!r},{d!r},{q:.6e},{th:.6e},{dn(q-th):.6e}\n")
    f.write(f"# Theorem A (mpmath.iv, separate certificate): Q(t) > 0 for all t >= 1736.452\n")
    f.write(f"# Union of rows proven (exact rational arithmetic) to cover [11, 1737] with no gaps.\n")
worst = min(dn(q-th) for _,_,_,q,th in certs)
print(f"[result] Q(t) >= 0 certified on [11, 1737]; worst certified margin = {worst:.3e}")
