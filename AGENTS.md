...
> 2. **Do not merge [PR #7](https://github.com/PrinceHoid/weil-gaussian-audit/pull/7) wholesale.** It was prepared from the old `main` and did not audit PR #6. Its live branch now preserves Route 004 and uses the resolved numbering below, but it must still be rebased after PR #6 and split into independently reviewable audit, candidate-tail, Route 005, and future-proposal changes.
...
> 4. **Preserve the resolved numbering.** Use **Route 004 — spectral convergence**, **Route 005 — all-scale Gaussian**, **Route 006 — de Bruijn–Newman**, **Route 007 — Nyman–Beurling/Báez–Duarte**, and **Route 008 — Robin/Lagarias**. Never recycle a rejected route number.
> 5. **Retain the factual corrections already applied to PR #7.** The published unconditional de Bruijn–Newman bound is [`Λ <= 0.20`](https://arxiv.org/abs/2004.09765); Route 004 remains active; and generic spectral heuristics must not be conflated with its precise convergence targets.
>
